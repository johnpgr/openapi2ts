import { arrowFunctionExpression, blockStatement, callExpression, classBody, classDeclaration, classMethod, classProperty, exportNamedDeclaration, exportSpecifier, expressionStatement, identifier, importDeclaration, importNamespaceSpecifier, logicalExpression, memberExpression, newExpression, objectExpression, objectProperty, program, returnStatement, spreadElement, stringLiteral, thisExpression, attachOptionalParameter, parameterDeclaration, tsQualifiedName, tsTypeAliasDeclaration, tsTypeAnnotation, tsTypeParameterInstantiation, tsTypeReference } from '../../emit/index.ts';
import type { ExportNamedDeclaration, Statement } from '../../emit/index.ts';
...
import type { OpenApiClientBuiltinBinaryType, OpenApiClientCustomizableBinaryType, OpenApiClientGeneratorConfig } from '../openapi-to-typescript-client.ts';
    ...
    return {
        data: renderTypeScript(
            program([
                ...
                clientClass,
                ...(typeExportSpecifiers.length > 0 ? [exportNamedDeclaration(undefined, typeExportSpecifiers, 'type')] : []),
                ...(valueExportSpecifiers.length > 0 ? [exportNamedDeclaration(undefined, valueExportSpecifiers)] : [])
            ]),
            commentsConfig
        ),
        filename
