        ...
        const importSpecifiers = Object.entries(imports).map(([alias, {kind, entity}]) => {
            const specifier =
                entity === 'default'
                    ...
                    : importSpecifier(identifier(entity.name), identifier(entity.name), !allTypes && kind === 'type');
            return specifier;
        });
        const declaration = importDeclaration(importSpecifiers, stringLiteral(path), allTypes ? 'type' : undefined);
