...
import { formatFilename } from "./string-utils.ts";
import type { FilenameFormat } from "./string-utils.ts";
...
export function getFilenameAndImportPath(
    ...
    if (config.filename) {
        partialResult = {
            ...
            importPath: config.filename.replace(
                /\.[jt]sx?$/,
                config.filenameFormat?.extension ?? "",
            ),
        };
    } else {
        partialResult = {
