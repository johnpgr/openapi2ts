...
test("parseCliArgs: quick --file --out", () => {
    const result = success(
        parseCliArgs(["generate", "--file", "./schema.json", "--out", "./src/api"]),
    );
...
test("parseCliArgs: quick --url -o", () => {
    const result = success(
        parseCliArgs(["check", "--url", "http://example.com/openapi.json", "-o", "./out"]),
    );
    assert.equal(result.command, "check");
    assert.equal(result.mode, "quick");
...
test("buildQuickConfig: defaults and optional fields", () => {
    ...
    assert.deepEqual(fromFile.generates[0]?.document.source, {
        type: "file",
        path: "./schema.json",
    });
